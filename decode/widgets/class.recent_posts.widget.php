<?php 
	class DC_recent_posts_widget extends WP_widget {
		function __construct () {
			$widget_ops = array(
				'classname' => 'dc_recent_posts_widget', 
				'description' => __( "Your site&#8217;s most recent Posts.", THEMENAME) );
			parent::__construct('dc_recent_posts_widget', 'Decode - ' . __('Recent Posts', THEMENAME), $widget_ops);
			$this->alt_option_name = 'dc_recent_posts_widget';
		}
		...
		public function widget( $args, $instance ) {
			$cache = array();
			if( ! $this->is_preview() ) {
				$cache = wp_cache_get( 'dc_recent_posts_widget', 'widget' );				
			}
	...
	<?php
		...
		public function update( $new_instance, $old_instance ) {
			...
			if ( isset($alloptions['dc_recent_posts_widget']) )
				delete_option('dc_recent_posts_widget');

			return $instance;
		}

		public function flush_widget_cache() {
			wp_cache_delete('dc_recent_posts_widget', 'widget');
		}
